...
export function MainDashboard({ onNavigateToReceiving, authSession }) {
  ...
  const realSending = activeReferrals.filter(r => r.originHospitalId === authSession?.hospitalId);
  const realReceiving = activeReferrals.filter(r => r.targetHospitalId === authSession?.hospitalId || r.acceptedHospitalId === authSession?.hospitalId);

  const sendingReferrals = realSending;
  const receivingReferrals = realReceiving;
